...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
     ...
     * 上拉加载是否可用
     */
    public boolean mPullLoadEnabled = true;
    /**
     * 判断滑动到底部加载是否可用
     */
     ...
     * 上拉的状态
     */
    protected State mPullUpState = State.NONE;
    /**
     * 可以下拉刷新的View
     */
    T mRefreshableView;
    ...
    public void showgif() {
        ImageView mMyGifView = mHeaderLayout.findViewById(R.id.MyGifView);
        mMyGifView.setImageResource(R.drawable.asd);
    }

    public void closeShowGif() {
        ImageView mMyGifView = mHeaderLayout.findViewById(R.id.MyGifView);
        if (mMyGifView != null) {
            mMyGifView.setImageResource(R.drawable.asd);
        }
    ...
    public void onPullUpRefreshComplete() {
        if (isPullLoading()) {
            ...
            if (mRefreshableView instanceof ListView) {
                ((ListView) mRefreshableView).smoothScrollBy(10, 1);
                ((ListView) mRefreshableView).smoothScrollBy(-10, 1);
            }
        }
    }
