...
//    private Animation mRotateAnimation;
    private boolean isShow=true;//是否显示头部的下拉布局
    private GifView headGiv;
    ...
    private void init(Context context) {
        ...
        ImageView myGridView= (ImageView) findViewById(R.id.MyGifView);//这里用图片控件也能加载gif动画
    //  myGridView.setMovieResource(R.drawable.asd);
        //加载动画
        Glide.with(getContext()).load(R.drawable.asd).into(myGridView);
        mArrowImageView.setScaleType(ScaleType.CENTER);
        // mArrowImageView.setImageResource(R.drawable.default_ptr_rotate);
      // Glide.with(context).load(R.drawable.asd).asGif().diskCacheStrategy(DiskCacheStrategy.SOURCE).placeholder(R.drawable.asd).into(mArrowImageView);
...
//        mRotateAnimation.setRepeatMode(Animation.RESTART);
    ...
    public int getContentSize() {
        if (null != mHeaderContainer && mHeaderContainer.getHeight() >0) {
